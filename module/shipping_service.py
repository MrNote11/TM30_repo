...
class ShippingService:
    ...
    def login(cls):
        ...
        shipping_logger.info(f"url: {url}, payload: {payload}, response: {response}")
    ...
    def get_states_with_stations(cls, **kwargs):
        ...
        # }
        # url = f"{settings.UPDATED_SHIPPING_BASE_URL}/location/state?StateName={kwargs.get('state_name')}"
        # if kwargs.get('city_name'):
        #     url = f"{settings.UPDATED_SHIPPING_BASE_URL}/location/state?StateName={kwargs.get('state_name')}&CityName={kwargs.get('city_name')}"
        ...
        shipping_logger.info(f"url: {url}, headers: {header}, response: {response}")
    ...
    def rating(cls, **kwargs):
        ...
        shipping_logger.info(f"payload ------->>>>> {payload}")
        admin_logger.info(f"payload ------->>>>> {payload}")
    ...
    def update_shipper(cls, pk, **kwargs):
        ...
        shipping_logger.info(f"url: {url}, headers: {header}, payload: {payload}, response: {response}")
    ...
    def pickup(cls, **kwargs):
        ...
        for order in all_order:
            order_number = f"PAYMALL-{order.order_id}{order.id}-{str(uuid.uuid4().int)[:4]}"
            if retry:
                order_number = f"PAYMALL-{order.order_id}{order.id}-{str(uuid.uuid4().int)[:4]}"

            seller = order.product_detail.product.store.seller
            ...
            detail = {
                ...
                "CompanyId": 643,  # Example, replace with dynamic value if needed

                "Summary": order.product_detail.product.description,
                "DeliveryRequestedTime": "06 AM to 09 PM",
                "PickupTime": str(pickup_time),
                "PickupDate": pickup_date,
                "PickupCoordinate": {
                    "Latitude": str(seller.latitude),
                    "Longitude": str(seller.longitude)
                },
                "PickupAddress": seller.get_full_address(),
                "SenderPhoneNumber": f"0{seller.phone_number[-10:]}",
                "SenderName": seller.user.get_full_name() if seller.user.get_full_name() else seller.user.email,
                "SenderEmail": seller.user.email,
                ...
                # "DeliveryTime": "4 PM to 7 PM",
                "ReceiverPhoneNumber": f"0{customer.phone_number[-10:]}",
                "ReceiverName": customer.user.get_full_name(),
                "PickupState": seller.state,
                "DeliveryState": address.state,
                "TotalWeight": order.product_detail.weight,

                "PickupStationId": 4,  # Example, replace with dynamic value if needed
                "DeliveryStationId": 4,  # Example, replace with dynamic value if needed
                "SenderTownId": "32",  # Example, replace with dynamic value if needed
                "ReceiverTownId": "45",  # Example, replace with dynamic value if needed
            ...
            for product in all_order.filter(shipper_name=order.shipper_name):
                shipment_item = {
                    "PackageId": product.id,
                    "Name": product.product_detail.product.name,
                    "Quantity": product.quantity,
                    "Color": product.product_detail.color,
                    "Size": product.product_detail.size,
                    "Description": product.product_detail.product.description,
                    "Distance": 5,
                    "WeightRange": "0",
                    ...
                    "DeliveryContactName": customer.user.get_full_name(),
                    "DeliveryContactNumber": f"0{customer.phone_number[-10:]}",
                    "DeliveryGooglePlaceAddress": address.get_full_address,
                    "DeliveryLandmark": " ",
                    "DeliveryState": address.state,
                    "DeliveryCity": address.city,
                    "PickupState": seller.state,
                ...
                detail["ShipmentItems"].append(shipment_item)

            shipment_information.append(detail)

        payload = {
            # "OrderNo": f"payarena-{kwargs.get('order_id', '0001')}",
            "Summary": kwargs.get("order_summary", "Order Summary"),
            "OrderInformation": shipment_information
        }

        payload = json.dumps(payload)
        url = f"{settings.UPDATED_SHIPPING_BASE_URL}/core/book"
        shipping_logger.info(f"url: {url}, payload: {payload}")
        admin_logger.info(f"url: {url}, payload: {payload}")
    ...
    def track_order(cls, tracking_id):
        ...
        url = f"{settings.UPDATED_SHIPPING_BASE_URL}/core/book/core/track?orderNo={tracking_id}"

...
class DashboardAPIView(APIView):
    ...
    def get(self, request):
        ...
        for order_product in latest_purchased_products:
            ...
            # Check if image and image.url exist
            if order_product.product_detail.product.image and order_product.product_detail.product.image.image:
                order["image"] = request.build_absolute_uri(order_product.product_detail.product.image.image.url)
            else:
                order["image"] = None
...
class ProductAPIView(APIView):
    ...
    def put(self, request, pk):
        try:
            ...
            if not success:
                return Response({"error": query}, status=status.HTTP_400_BAD_REQUEST)
...
class ProductListAPIView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if query:
            if query.isdigit():
                queryset = Product.objects.filter(id__iexact=query).distinct()
                ...
                ).filter(rank__gte=0.1).order_by('-rank', '-sale_count', '-view_count', '-created_on')

                # Step 2: Fallback to Trigram Similarity if full-text search returns no results
                if not search_products.exists():
                    search_products = Product.objects.annotate(
                        similarity=TrigramSimilarity('name', query)
                    ).filter(similarity__gt=0.1).order_by('-similarity', '-sale_count', '-view_count', '-created_on')

                return search_products.distinct()
...
class ProductCategoryListAPIView(generics.ListCreateAPIView):
    ...
    queryset = ProductCategory.objects.filter(parent=None).order_by('-id')
    pagination_class = CustomPagination
    filter_backends = [DjangoFilterBackend, SearchFilter]
    search_fields = ["name"]
    ...
    def create(self, request, *args, **kwargs):
        try:
            ...
            if not image:
                return Response({"detail": "Image must be added to proceed."}, status=status.HTTP_400_BAD_REQUEST)

            resized_image = resize_and_save_image(image, 200, 200)
            ...
            category = create_or_update_category(data=request.data, image=resized_image)
...
class ProductCategoryDetailRetrieveAPIView(generics.RetrieveUpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        category = create_or_update_category(data=request.data, image=resized_image, cat_id=cat_id)
        return Response(ProductCategorySerializer(category, context={"request": request}).data)
...
class AdminSellerAPIView(APIView, CustomPagination):
    ...
    def post(self, request):
        ...
        try:
            ...
            # )
            # if not success:
            #     log_request(f"PayArena registration failed: {detail}")
...
class UpdateMerchantStatusAPIView(APIView):
    ...
    def put(self, request):
        ...
        declined_reason = request.data.get("declined_reason", None)
        ...
        if seller_status not in dict(seller_status_choices).keys():
            return Response({"error": f"Invalid seller status: {seller_status}. The available are {[_instance for _instance in dict(seller_status_choices).keys()]}"},
                            status=status.HTTP_400_BAD_REQUEST)

        try:
            seller = Seller.objects.get(id=seller_id)
            seller.status = seller_status
            seller.declined_reason = declined_reason
            ...
            if seller_status == "approve":
                ...
                admin_logger.info("Commence u_map_registration")
                ...
                admin_logger.info("Conclude u_map_registration")
                ...
                admin_logger.info("Commence merchant_account_approval_email, merchant_upload_guide_email")
            ...
            if seller_status == "active" and seller.biller_code == "":
                return Response(
                    {"detail": "Kindly approve merchant first, as biller code, is not registered"},
                    status=status.HTTP_400_BAD_REQUEST
                )
            ...
            if seller_status == "inactive" or seller_status == "declined":
                Store.objects.filter(seller=seller).update(is_active=False)
                # Send Disapproval Email to seller
                merchant_logger.info("Commence merchant_account_disapproval_email")
                admin_logger.info("Commence merchant_account_disapproval_email")

                Thread(target=merchant_account_disapproval_email, args=[seller.user.email, declined_reason]).start()

            return Response({"detail": "Merchant status updated successfully"})
        except Exception as ex:
            merchant_logger.error(f"Error in UpdateMerchantStatusAPIView: {ex}")
            admin_logger.error(f"Error in UpdateMerchantStatusAPIView: {ex}")
...
class AdminPasswordResetView(APIView):
    ...
    def put(self, request):
        try:
            ...
            admin_logger.error(f"Error in AdminPasswordResetView: {err}")
            # Log

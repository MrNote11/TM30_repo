...
class MerchantDashboardOrderProductSerializer(serializers.ModelSerializer):
    ...
    def get_order_id(self, obj):
        return obj.order.id if obj else None

    def get_customer_name(self, obj):
        if obj and obj.order and obj.order.customer:
            return f"{obj.order.customer.user.last_name} {obj.order.customer.user.first_name}"
        return None
    ...
    def get_product_name(self, obj):
        return obj.product_detail.product.name if obj.product_detail.product else None
...
class MerchantReturnedProductSerializer(serializers.ModelSerializer):
    ...
    def get_product_image(self, obj):
        ...
        return obj.product.product_detail.product.image.get_image_url() or None
...
class MerchantBannerSerializerIn(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        resized_image = resize_and_save_image(image, 500, 500)
        if not resized_image:
            raise serializers.ValidationError({"detail": "No valid images were uploaded."})

        if user.is_staff and seller:
            seller = Seller.objects.get(id=seller)
            banner = MerchantBanner.objects.create(seller=seller, banner_image=resized_image, is_active=is_active)
        else:
            seller = Seller.objects.get(user=user)
            # Delete Existing Banners
            MerchantBanner.objects.filter(seller=seller).delete()
            banner = MerchantBanner.objects.create(seller=seller, banner_image=resized_image)
    ...
    def update(self, instance, validated_data):
        ...
        if image:
            instance.banner_image = resized_image
        # if size:

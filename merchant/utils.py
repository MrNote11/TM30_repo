    ...
    try:
        ...
        if ext == ".eps":
            ...
            try:
                ...
                merchant_logger.error(f"Error processing EPS file: {e}")
                admin_logger.error(f"Error processing EPS file: {e}")
                return None
        else:
            try:
        ...
        merchant_logger.info(f"Processing image: {input_image.name}, size: {original_width}x{original_height}")
        admin_logger.info(f"Processing image: {input_image.name}, size: {original_width}x{original_height}")
        ...
        if original_width > width or original_height > height:
            ...
            merchant_logger.info(f"Resized size: {img.size}")
            admin_logger.info(f"Resized size: {img.size}")
        ...
        merchant_logger.error(f"Error resizing image: {e}")
        admin_logger.error(f"Error resizing image: {e}")
        return None


def create_product(request, seller):
    # Ensure only approved sellers can add products
    if seller.status not in ["approve", "active"]:
        return False, "Only approved sellers can add products.", None
    ...
    required_fields = {
        ...
        "sub-category ID": data.get("sub_category_id"),
        "product type ID": data.get("product_type_id"),
    ...
    if missing_fields:
        return (False, f"The following fields are required and missing: {', '.join(missing_fields)}", None,
        )

    # Fetch related models
    try:
        category = ProductCategory.objects.get(id=required_fields["category ID"])
        sub_category = ProductCategory.objects.get(id=required_fields["sub-category ID"])
        prod_type = ProductType.objects.get(id=required_fields["product type ID"])
        brand = Brand.objects.get(id=required_fields["brand ID"])
    except (ProductCategory.DoesNotExist, ProductType.DoesNotExist, Brand.DoesNotExist) as ex:
    ...
    product, created = Product.objects.get_or_create(
        ...
        defaults={"description": required_fields["description"], "brand": brand}
    )
    if not created:
    ...
    # Handle tags
    if required_fields["tags"]:
        product.tags = ", ".join(required_fields["tags"])
        product.save()

    # Product Details
    product_details = data.get("product_details", {})
    if product_details:
        success, detail = add_or_update_product_details(product_details, product)
        if not success:
...
def add_or_update_product_details(details, product):
    try:
        sku = details.get("sku", "")
        ...
        try:
            discount = float(details.get("discount", 0.0))
            price = float(details.get("price", 0.0))
            stock = int(details.get("stock", 0))
        ...
        try:
            ...
            return False, f"Invalid dimension input: {e}"

        try:
            low_stock_threshold = (
                int(details.get("low_stock_threshold")) if details.get("low_stock_threshold") is not None else None
            )
        except (ValueError, TypeError) as e:
            return False, f"Invalid low stock threshold input: {e}"

        detail_images = details.get("images", [])
        ...
        for field_name, value in {
            ...
            if value < 0:
                return False, f"Invalid value for {field_name}: cannot be negative."
        ...
        if not created:
            ...
            product_detail.width = width
            product_detail.height = height
            product_detail.low_stock_threshold = low_stock_threshold
            product_detail.save()

            # Handle detail images
            if detail_images:
                success, message = add_product_detail_images(detail_images, product_detail)
                if not success:
                    return False, message

        return True, "Product details added or updated successfully"
    except Exception as ex:
        merchant_logger.error(f"Error updating product details: {ex}")
        admin_logger.error(f"Error updating product details: {ex}")
        return False, f"An error occurred while updating product details: {ex}"


def add_product_detail_images(images, product_detail):
    """
    Adds already optimized images to the product detail.

    Args:
        images (list): List of pre-optimized image file objects.
        product_detail (ProductDetail): The associated ProductDetail object.
    """
    try:
        if not isinstance(images, list):
            raise ValueError("The images parameter must be a list of image objects.")
        ...
        for image_obj in images:
            ...
            if not image_id:
                raise ValueError("Image ID is missing in the provided data.")
            try:
                image = Image.objects.get(pk=image_id)
                product_images.append(ProductImage(product_detail=product_detail, image=image))
            except Image.DoesNotExist:
                merchant_logger.error(f"Image with ID {image_id} does not exist.")
                admin_logger.error(f"Image with ID {image_id} does not exist.")
            except Exception as inner_ex:
                merchant_logger.error(f"Error processing image object {image_obj}: {inner_ex}")
                admin_logger.error(f"Error processing image object {image_obj}: {inner_ex}")

        if product_images:
            ProductImage.objects.bulk_create(product_images)

        merchant_logger.info(f"Successfully added {len(product_images)} images for ProductDetail ID {product_detail.id}")
        admin_logger.info(f"Successfully added {len(product_images)} images for ProductDetail ID {product_detail.id}")
        return True, f"{len(product_images)} images added successfully."

    except Exception as ex:
        merchant_logger.error(f"Error adding product detail images: {ex}")
        admin_logger.error(f"Error adding product detail images: {ex}")
        return False, f"Error adding product detail images: {ex}"
...
def update_product(request, product):
    ...
    direct_fields = {
        "name": data.get("name"),
        "status": data.get("status") if request.user.is_staff else None,
    ...
    # Handle category, sub-category, product type, and brand
    related_models = {
        "category_id": ProductCategory,
        "sub_category_id": ProductCategory,
    ...
    for field, model in related_models.items():
        if field in data:
            try:
                instance = model.objects.get(pk=data[field])
                setattr(product, field.replace("_id", ""), instance)
            except model.DoesNotExist:
                return False, f"Invalid {field} provided", None
    ...
    if "status" in data and request.user.is_staff:
        ...
        elif product.status == "approve":
            product.approved_by = request.user
            product.status = "active"
            product.published_on = datetime.datetime.now()
        elif product.status == "checked":
...
def validate_bank_details(account_number: str, account_name: str, bank_code: str, request):
    """
        Validate Bank details.
    """
    profile = Profile.objects.get(user=request.user)
    response = PayArenaServices.name_enquiry_from_bank(profile, bank_code, account_number)

    if response["Success"] is not True:
        error_response = response
        merchant_logger.error(f"Error in validate_bank_details: {error_response}")
        admin_logger.error(f"Error in validate_bank_details: {error_response}")

        return False, "Error while requesting for name enquiry"

    elif "Error" in response:
        return False, "Your request could not be processed at the moment, please try again later"

    account_name = account_name.lower().split(" ")
    response_name = (response["Data"]["BeneficiaryName"]).lower().strip()
    merchant_logger.info(f"Response from validate_bank_details: {response}")
    admin_logger.info(f"Response from validate_bank_details: {response}")
...
def create_seller(request, user, email, phone_number):
    ...
    try:
        ...
        # ---------------------------- Check Bank Details ----------------------------
        merchant_logger.info(f"Commence validate_bank_details")
        admin_logger.info(f"Commence validate_bank_details")
        success, msg = validate_bank_details(account_number=bank_account_number, account_name=bank_account_name,
                                             bank_code=bank_code, request=request)
        merchant_logger.info(f"Conclude validate_bank_details: {success}. {msg}")
        admin_logger.info(f"Conclude validate_bank_details: {success}. {msg}")
        if not success:
            merchant_logger.error(f"Failed validate_bank_details: {msg}")
            admin_logger.error(f"Failed validate_bank_details: {msg}")
            return False, msg

        # Get bank name
        auth_user = Profile.objects.get(user=request.user)

        merchant_logger.info(f"Commence get_all_banks commence")
        success, detail = get_all_banks(auth_user)
        admin_logger.info(f"Conclude get_all_banks: {success}. {detail}")
        ...
        # -------------------------------------------------------------------------------------
        # Atomic transaction for creating all associated records
        merchant_logger.info("Commence transaction block")
        admin_logger.info("Commence transaction block")
        with transaction.atomic():
            ...
            merchant_logger.info(f"User updated: {user.id}")
            admin_logger.info(f"User updated: {user.id}")

            # Update or create the Seller
            ...
            merchant_logger.info(f"Seller updated or created: {seller.id}")
            admin_logger.info(f"Seller updated or created: {seller.id}")

            # Update or create SellerDetail
            seller_detail, _ = SellerDetail.objects.update_or_create(
                seller=seller,
                defaults={
                    "market_size": int(request.data.get("market_size", None)),
                    "business_type": business_type,
                    "number_of_outlets": int(request.data.get("number_of_outlets", None)),
                    "maximum_price_range": int(request.data.get("maximum_price_range", None)),
            ...
            merchant_logger.info(f"SellerDetail updated or created: {seller_detail.id}")
            admin_logger.info(f"SellerDetail updated or created: {seller_detail.id}")
            ...
            merchant_logger.info(f"BankAccount updated or created: {bank_account.id}")
            admin_logger.info(f"BankAccount updated or created: {bank_account.id}")

            # Update or create Store
            store, _ = Store.objects.update_or_create(
                seller=seller,
                defaults={"name": business_name.capitalize()}
            )
            merchant_logger.info(f"Store updated or created: {store.id}")
            admin_logger.info(f"Store updated or created: {store.id}")
            ...
            if business_type == "unregistered-individual-business":
                ...
                if company_type not in ['sole-proprietorship', 'partnership']:
                    return False, "The available are sole-proprietorship & partnership"
                ...
                if not int(request.data.get("market_size", None)):
                    return False, "Market size is required"

                if not int(request.data.get("number_of_outlets", None)):
                    return False, "Number of outlet is required"

                if not int(request.data.get("maximum_price_range", None)):
        ...
        import traceback
        merchant_logger.error(f"FULL EXCEPTION: {err}")
        admin_logger.error(f"FULL EXCEPTION: {err}")
        merchant_logger.error(f"DETAILED EXCEPTION: {traceback.format_exc()}")
        admin_logger.error(f"DETAILED EXCEPTION: {traceback.format_exc()}")
...
def bulk_upload_thread(user, upload):
    ...
    for row in read:
        try:
            ...
            merchant_logger.info(f"Product Name: {name} created successfully")
            admin_logger.info(f"Product Name: {name} created successfully")

        except Exception as ex:
            error_list.append(f"Product Name: {name} - {ex}")
            merchant_logger.error(f"Product Name: {name} - {ex}")
            admin_logger.error(f"Product Name: {name} - {ex}")

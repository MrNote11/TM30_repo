...
# print(token.key)
hashids = Hashids(salt=settings.HASHIDS_SALT, min_length=8)  # Use a secure salt and set minimum length
...
class Seller(models.Model):
    ...
    declined_reason = models.CharField(max_length=200, blank=True, null=True)
    # is_declined = models.BooleanField(default=False)  # New field

    follower = models.ManyToManyField(User, blank=True, related_name='merchant_follower_list')

    def __str__(self):
        return f'{self.user}-{self.phone_number}: {self.status}'

    def get_full_address(self):
        components = [self.address, self.town, self.city, self.state]
        return ', '.join(filter(None, components))

    @property
    def merchant_code(self):
        """Encode the user ID into a unique code."""
        return hashids.encode(self.user.id)

    @classmethod
    def decode_merchant_code(cls, code):
        """Decode the unique code back into a user ID."""

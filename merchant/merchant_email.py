...
def merchant_order_placement_email(customer, order, order_product):
    ...
    email = order_product.product_detail.product.store.seller.user.email
    ...
    buyer_logger.info(f"===========================================")
    admin_logger.info(
        f"merchant_order_placement_email begin sending email to: {customer}, order_product: {order_product}")
    buyer_logger.info(
        f"merchant_order_placement_email begin sending email to: {customer}, order_product: {order_product}")
    admin_logger.info(f"===========================================")
    buyer_logger.info(f"===========================================")
...
def merchant_order_delivered_email(customer, order_id, order_product):
    ...
    buyer_logger.info(f"===========================================")
    admin_logger.info(
        f"merchant_order_delivered_email begin sending email to: {customer}, order_product: {order_product}")
    buyer_logger.info(
        f"merchant_order_delivered_email begin sending email to: {customer}, order_product: {order_product}")
    admin_logger.info(f"===========================================")
    buyer_logger.info(f"===========================================")
...
def merchant_account_disapproval_email(email, reason):
    contents = render(None, 'merchant_account_disapproved.html', {'reason': reason}).content.decode('utf-8')
    subject = "Merchant Registration Disapproved!"
    send_email(contents, email, subject)
    return True

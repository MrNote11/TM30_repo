...
def send_shopper_verification_email(email, profile, source):
    try:
        ...
        buyer_logger.error(f"Error {err}")
        admin_logger.error(f"Error {err}")
        # LOG ERROR
        return False


def create_account(email, phone_number, password, first_name, surname,
                   # gender, date_of_birth
                   ):
    try:
        user_instance = User.objects.create(
            username=email, email=email, password=make_password(password), first_name=first_name, last_name=surname,
...
def register_payarena_user(email, username, phone_number, f_name, surname,
                           # date_of_birth, gender,
                           password, password_confirm):
    # Check if user is available on Payarena Portal
    phone_no = f"0{phone_number[-10:]}"
    response = PayArenaServices.register(
        email=email, username=username, phone_no=phone_no, first_name=f_name, surname=surname, password=password,
        # date_of_birth=date_of_birth, gender=gender,
        password_confirm=password_confirm
    )
    if "Success" in response:
        if response["Success"] is False:
            if response["Messages"] == f"User name '{email}' is already taken.":
                pass
            else:
                return False, str(response["Messages"])
...
#                 # date_of_birth = response["Data"]["userProfile"]["DateOfBirth"]
#                 user = User.objects.create(username=user_email, first_name=first_name, last_name=last_name,
#                                            password=make_password(password), email=user_email,
#                                            # gender=gender,
...
def login_payarena_user(profile, email, password):
    ...
    if not response or "Success" not in response or not response["Success"]:
        raise ValueError("PayArena login failed. No valid response received.")

    user_token = response['Data'].get("access_token")

    if not user_token:
        raise ValueError("PayArena login failed. No access token returned.")

    if profile is None:
        user_data = response["Data"]["userProfile"]
        user_email = user_data["Email"]
        first_name = user_data["FirstName"]
        last_name = user_data["Surname"]
        phone_no = user_data["PhoneNumber"]
    ...
    # 🔥 Ensure encryption only happens if user_token is valid
    if user_token:
        encrypted_token = encrypt_text(user_token)  # ✅ Encrypt only if token is available
        profile.pay_auth = encrypted_token
        profile.save()
    else:
        # Log or handle case where token is missing (should not happen)
        raise ValueError("PayArena login failed: No valid token received.")
...
def change_payarena_user_password(profile, old_password, new_password):
    ...
    if "Success" in response:
        if response["Success"] is False:
            buyer_logger.error(f"Error logging request on User Engine. error_message: {response['Message']}")
            admin_logger.error(f"Error logging request on User Engine. error_message: {response['Message']}")
            return False, f"{response['Message']}"
        else:
            buyer_logger.info(f"Password changed on User Engine message: {response['Message']}")
            admin_logger.info(f"Password changed on User Engine message: {response['Message']}")
...
def collecting_commissions(request):
    pass
...
def confirm_or_create_billing_account(profile, email, password):
    ...
    if "error" in response:
        ...
        buyer_logger.info(f"Billing account created for {f_name} {l_name} response: {result}")
        admin_logger.info(f"Billing account created for {f_name} {l_name} response: {result}")
        return True
    return True
...
def reset_password(pin, password, email, user):
    ...
    admin_logger.info(f"response from PayArenaServices: {response}")
    if "Success" in response:
        if response["Success"] is True:
